...
program hmipt_matsuara
  ...
  do while(.not.converged.AND.iloop<nloop)
     ...
     !call broyden_mix(L,dimag(fg0),dimag(GFold),weight,5)
     !
     !IMPURITY SOLVER
  ...
  call fftgf_iw2tau(fg,gt(0:),beta)
  open(100,file="fft_sigma_iw.ipt")
  open(101,file="fft_g_tau.ipt")
  do i=0,L
     write(100,*)i*beta/dble(L),sigt(i)
     write(101,*)i*beta/dble(L),gt(i)
  enddo
  close(100)
  close(101)
  ...
  fg=zero
  do i=1,L
     fg(i) = 0.5d0/(xi*wm(i) - U/2.d0) + 0.5d0/(xi*wm(i) + U/2.d0)
  enddo
  call splot("G_atomic_iw.ipt",wm,fg)
  call fftgf_iw2tau(fg,gt(0:),beta)
  call splot("G_atomic_tau.ipt",(/(i*beta/dble(L),i=0,L)/),gt(0:))
